...
pub fn IndexPoolType(
    ...
    return struct {
        ...
        pub fn lookup(index_pool: *IndexPool, key: Key) !*PoolLookupResult {
            ...
            var result_block_count: IndexPtr = 0;
...
test "OrderItemRows:order_id index:pool:find" {
    ...
    const index_pool: *IndexPoolType(table_ptr_list.len, keys_per_block) = try .init(allocator, table_ptr_list);
...
test "OrderItemRows:order_id index:pool" {
    const allocator = std.testing.allocator;
